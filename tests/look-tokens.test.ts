...
describe("look tokens and mood control", () => {
  ...
  it("defines four look token blocks", () => {
    expect(css).toContain('[data-look="day"]');
    expect(css).toContain('[data-look="night"]');
    expect(css).toContain('[data-look="thea"]');
    expect(css).toContain('[data-look="remnote"]');
  });

  it("keeps night desk oklch values including background 0.155 0.012 55", () => {
    ...
    expect(nightBlock).toContain("oklch(0.155 0.012 55)");
    expect(nightBlock).toContain("oklch(0.78 0.12 75)");
    expect(css).toContain("oklch(0.155 0.012 55)");
  });

  it("uses the exact mood copy and Day, Night, Thea-Style, RemNote-Style order", () => {
    expect(mood).toContain("Change today's mood");
    const day = mood.indexOf('"Day"');
    const night = mood.indexOf('"Night"');
    const thea = mood.indexOf('"Thea-Style"');
    const remnote = mood.indexOf('"RemNote-Style"');
    expect(day).toBeGreaterThan(-1);
    expect(night).toBeGreaterThan(day);
    expect(thea).toBeGreaterThan(night);
    expect(remnote).toBeGreaterThan(thea);
    expect(shell).toContain("MoodControl");
  });

  it("defaults to night, persists omni-look, and toggles dark only for night", () => {
    expect(provider).toContain('LookId = "day" | "night" | "thea" | "remnote"');
    expect(provider).toContain('"day"');
    expect(provider).toContain('"night"');
    expect(provider).toContain('"thea"');
    expect(provider).toContain('"remnote"');
    ...
    expect(layout).toContain('--font-thea');
    expect(layout).toContain('--font-remnote');
  });
